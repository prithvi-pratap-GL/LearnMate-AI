...
async def submit_round_1(request: Round1SubmissionRequest):
    ...
    try:
        ...
        roadmap_content = await roadmap_service.generate_roadmap(level, strengths, weak_areas, score, sanitized_topic)
        ...
        if score < 50:
            ...
            content = await challenge_service.generate_performance_analysis(sanitized_topic, questions_dict, score)
        ...
        raise HTTPException(status_code=500, detail="An internal error occurred during evaluation.")


def _extract_keywords(question: str) -> set:
    """Extract key technical terms from a question for similarity detection."""
    ignore_words = {"what", "is", "a", "the", "an", "in", "of", "to", "for", "or", "and", "by", "with", "on", "at", "this", "that", "are", "does", "do", "can", "which", "how", "you", "your", "used"}
...
async def generate_round_2_questions(http_request: Request):
    ...
    try:
        ...
        filtered_questions = [
            ...
            and not any(_are_questions_similar(q.get("question", ""), r1q) for r1q in round_1_q_texts)
        ]
        ...
        while len(all_questions) < 5 and attempts < 4:
            ...
            for q in more_questions:
                if (q.get("question", "").lower() not in round_1_q_texts
                    ...
                    and not any(_are_questions_similar(q.get("question", ""), aq.get("question", "")) for aq in all_questions)):
        ...
        if len(final_questions) < 5:
            log.warning(f"Could not generate 5 unique advanced questions for topic '{topic}'. Got {len(final_questions)}.")

        return {"questions": final_questions, "round": 2, "total_questions": len(final_questions)}
    except Exception as e:
        log.error(f"Error in /generate-round-2-questions: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to generate Round 2 questions.")
...
async def submit_round_2(request: Round2SubmissionRequest):
    ...
    try:
        ...
        sanitized_topic = sanitize_input(request.topic)
        questions_dict = [q.model_dump() if hasattr(q, 'model_dump') else q for q in questions_list]
        content = await challenge_service.generate_solution_explanation(sanitized_topic, questions_dict, score)
        ...
        raise HTTPException(status_code=500, detail="An internal error occurred during the final analysis.")
...
async def analyze_learning(request: LearningAnalysisRequest):
    ...
    try:
        ...
        if score < 50:
            ...
            content = await challenge_service.generate_advanced_challenges(sanitized_topic, strengths, score, level)
